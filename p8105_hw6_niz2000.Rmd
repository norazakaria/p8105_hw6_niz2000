...
```{r Test for Missing Values}
missing_test = birthweight %>%
  summarise_all((funs(sum(is.na(.))))) 
missing_test
```

After tidying, the missing_test dataframe was created to detect if there are any missing values in the dataset. As we see in the missing_test outputted table, there are 0 missing values in each of the variable columns.
...
quantile(pull(weather_bootstrap, intercept_beta1), probs = c(.025, 0.975))
